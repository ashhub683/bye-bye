...
MBCI_OTHER_ARGS = [
    'token', 'chat_id', 'skip_webdriver_menu', 'no_headless', 'custom_browser_location', 'custom_email_api',
...
def parse_argv(sys_argv=None):
    ...
    if MBCI_MODE and sys_argv is None: # for MBCI mode
        ...
        # Optional
        args_parser.add_argument('--token', type=str, help='Telegram Bot Token for sending files')
        args_parser.add_argument('--chat_id', type=str, help='Telegram Chat ID for sending files')
...
def send_to_telegram(token, chat_id, file_path):
    ...
    try:
        ...
        if response.status_code == 200:
            logging.info(f"File {file_path} sent to Telegram successfully")
            console_log(f"File {file_path} sent to Telegram successfully", OK)
        else:
            logging.error(f"Failed to send file. Status code: {response.status_code}")
            console_log(f"Failed to send file. Status code: {response.status_code}", ERROR)
